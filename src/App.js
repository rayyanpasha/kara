...
const RouteIcon = () => <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="h-5 w-5 mr-2"><path d="M12 2L12 12M12 12L12 22M12 12L22 12M12 12L2 12"/></svg>;
...
const ThreeBackground = () => {
    ...
    useEffect(() => {
        ...
        // Improved smoother transition for earthGroup position and rotation between steps 0 and 1
        tl.to(earthGroup.position, { x: 0, y: 1, z: -1, ease: "power2.inOut", duration: 0.5 }, 0.5)
          .to(earthGroup.rotation, { y: Math.PI * 1.5, z: 0.15, ease: "power2.inOut", duration: 0.5 }, 0.5);
    }
...
const RoutePlanner = ({ apiKey }) => {
    ...
    const handleFindRoute = async () => {
        ...
        // 1. Set Google Maps URL
        const encodedStart = encodeURIComponent(`${startPoint}, Bangalore`);
        const encodedEnd = encodeURIComponent(`${endPoint}, Bangalore`);
        setMapUrl(`https://www.google.com/maps/embed/v1/directions?key=YOUR_GOOGLE_MAPS_API_KEY&origin=${encodedStart}&destination=${encodedEnd}`);

        // 2. Generate AI Route
        setIsGenerating(true);
        setAiRoute('');
        const prompt = `As an expert in Bengaluru city planning and air quality, generate a healthier, low-pollution travel route for a teenager from "${startPoint}" to "${endPoint}". The standard route might use main roads. Propose an alternative that prioritizes quieter streets, roads with more trees, or paths through parks, even if it's slightly longer. Explain WHY this route is better (e.g., "avoids the congested Hosur Road," "cuts through the greener Cubbon Park"). Provide clear, turn-by-turn directions.`;
        ...
        try {
            ...
            const result = await response.json();
            if (result.candidates && result.candidates[0]?.content?.parts[0]?.text) {
                setAiRoute(result.candidates[0].content.parts[0].text);
            } else {
                setAiRoute("Could not generate an alternative route at this time.");
            }
        } catch (error) {
            console.error("AI Route generation failed:", error);
            setAiRoute("An error occurred while generating the healthier route.");
        } finally {
    ...
    return (
        <div className="space-y-6">
            <div className="bg-slate-800/50 p-4 rounded-xl border border-slate-700 space-y-4">
                <h3 className="text-xl font-bold text-white">Low-Pollution Route Planner</h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <input type="text" value={startPoint} onChange={(e) => setStartPoint(e.target.value)} placeholder="Start Point (e.g., Koramangala)" className="w-full bg-slate-700 border border-slate-600 rounded-lg px-3 py-2 text-white" />
                    <input type="text" value={endPoint} onChange={(e) => setEndPoint(e.target.value)} placeholder="End Point (e.g., Indiranagar)" className="w-full bg-slate-700 border border-slate-600 rounded-lg px-3 py-2 text-white" />
                </div>
                <button onClick={handleFindRoute} disabled={isGenerating} className="w-full bg-green-600 text-white font-bold py-2 px-4 rounded-lg hover:bg-green-700 flex items-center justify-center disabled:bg-slate-500">
                    {isGenerating ? <GeneratingSpinner text="Finding Healthier Route..." /> : <><RouteIcon /> Find Healthier Route</>}
                </button>
            </div>
            ...
            {mapUrl && (
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    <div className="bg-slate-800/50 p-2 rounded-xl border border-slate-700">
                        <h4 className="font-bold text-white p-2">Standard Route Map</h4>
                        <iframe
                            width="100%"
                            height="450"
                    ...
                    <div className="bg-slate-800/50 p-4 rounded-xl border border-slate-700">
                        <h4 className="font-bold text-white mb-2 flex items-center"><SparklesIcon /> AI-Suggested Healthier Route</h4>
                        {isGenerating ? (
                            <GeneratingSpinner />
                        ) : (
                            <div className="text-slate-300 whitespace-pre-wrap text-sm leading-relaxed" dangerouslySetInnerHTML={{ __html: aiRoute.replace(/\*\*(.*?)\*\*/g, '<strong class="text-indigo-300">$1</strong>').replace(/\n/g, '<br />') }}></div>
                        )}
                    </div>
                </div>
            )}
